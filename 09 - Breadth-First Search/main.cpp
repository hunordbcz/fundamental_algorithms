...
enum{
    ...
    COMMAND_PATH,
    COMMAND_PERF,
    COMMAND_NEIGHB_KNIGHT,
    COMMAND_BFS_KNIGHT,
    COMMAND_BFS_KNIGHT_STEP
};
...
int readGrid(Grid *grid)
{
    ...
    int i, j;
    printf("Choose one:\n");
    printf("Grid (1) - Default\n");
    printf("Empty Chessboard (2)\n");
    printf("Chessboard with Obstacles (3)\n");
    char fileName[30];
    char gridType = 0;
        gridType = _getch();
        string text;
        if (gridType == '2') {
            sprintf(fileName, "%s", "chessboard.txt");
        }
        else if (gridType == '3') {
            sprintf(fileName, "%s", "chessboard_obstacles.txt");
        }
        else {
            sprintf(fileName, "%s", "grid.txt");
        }
...
void readCommand(COMMAND *c, const Grid *grid)
{
    ...
    do{
        ...
        if(strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 || strcmp(token, "q") == 0){
            ...
            validCommand = 1;
        }else if(strcmp(token, "neighb") == 0 || strcmp(token, "neighb_knight") == 0 || strcmp(token, "bfs_knight") == 0 || strcmp(token, "bfs_knight_step") == 0 || strcmp(token, "neighbors") == 0 ||
            ...
            }else if(strcmp(token, "neighb_knight") == 0){
                c->command = COMMAND_NEIGHB_KNIGHT;
            }else if (strcmp(token, "bfs_knight_step") == 0) {
                c->command = COMMAND_BFS_KNIGHT_STEP;
            }else if (strcmp(token, "bfs_knight") == 0) {
                c->command = COMMAND_BFS_KNIGHT;
...
int main()
{
    ...
    for(;;){
        ...
        if(cmd.command == COMMAND_EXIT){
        ...
        } else if (cmd.command == COMMAND_NEIGHB_KNIGHT) {
            grid2 = grid;
            Point neighb[4];
            int nrNeighb = get_neighbors_knight(&grid, cmd.p1, neighb);
            grid2.mat[cmd.p1.row][cmd.p1.col] |= MASK_START;
            if (grid.mat[cmd.p1.row][cmd.p1.col] & MASK_WALL) {
                snprintf(err, 100, "Starting position (%d, %d) is inside a wall.", cmd.p1.row, cmd.p1.col);
                errors.push_back(err);
        ...
        } else if(cmd.command == COMMAND_BFS || cmd.command == COMMAND_BFS_KNIGHT || cmd.command == COMMAND_BFS_KNIGHT_STEP || cmd.command == COMMAND_BFS_STEP || cmd.command == COMMAND_BFS_TREE){
            grid2 = grid;
            boolean knight = false;
            if (cmd.command == COMMAND_BFS_KNIGHT || cmd.command == COMMAND_BFS_KNIGHT_STEP) {
                knight = true;
            }
            grid_to_graph(&grid, &graph, knight);
            ...
            if(s == NULL){
                ...
                if(cmd.command == COMMAND_BFS || cmd.command == COMMAND_BFS_TREE || cmd.command == COMMAND_BFS_KNIGHT){
                    for(i=0; i<graph.nrNodes; ++i){
                        if(graph.v[i]->color == COLOR_BLACK){
                            Point p1;
                            p1 = graph.v[i]->position;
                ...
                }else if(cmd.command == COMMAND_BFS_STEP || cmd.command == COMMAND_BFS_KNIGHT_STEP){
                    int newLayer = 1;
                    int crtDist = 0;
                    displayGrid(&grid2, cmd.command);
                    while(newLayer){
            ...
            grid2 = grid;
            grid_to_graph(&grid, &graph, false);
